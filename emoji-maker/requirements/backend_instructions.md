...
1. Create user-to-user table
    ...
    1. After a user signs in via Clerk, we should get the userId from Clerk, and check if this userId exists in the 'profiles' table.
    2. If the user doesn't exist, then create a user in the 'profiles' table.
    3. If the user already exists, then proceed and pass on user_id to functions like generating emojis.
